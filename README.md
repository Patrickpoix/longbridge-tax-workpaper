...
**什么情况需要提供历史月结单？**
- 某只标的在纳税年初已有持仓，且纳税年度内**没有新的买入记录**
- 此时系统缺少该标的的原始买入数据来重建 FIFO 成本
- 需要提供该标的**开仓以来**的月结单（例如某港股 2024 年 8 月买入，则需 2024 年 8 月起的数据）

**不提供会怎样？**
- 系统不会把成本算作 0——长桥月结单本身就展示成本（移动平均口径）
- 系统会降级使用券商展示成本作为替代
- 复核状态中会标记为 `WARNING`，列出具体缺失记录的标的

**什么情况不需要？**
- 选择 MOVING_AVERAGE（默认）模式：直接使用券商展示成本，**无需任何历史月结单**
- 选择 FIFO 或 BOTH：如果标的在纳税年度内有买入记录，系统直接用当年买入重建成本，无需追溯
